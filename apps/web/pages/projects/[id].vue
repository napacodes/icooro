...
async function loadProject() {
  ...
  try {
    ...
    const [episodeResponse, characterResponse, locationResponse, propResponse] = await Promise.all([
      ...
      request<ListResponse<Asset>>(`/characters`), request<ListResponse<Asset>>(`/locations`), request<ListResponse<Asset>>(`/props`),
    ]);
    episodes.value = episodeResponse.data;
    characters.value = characterResponse.data.filter((item) => item.projectId === route.params.id);
    locations.value = locationResponse.data.filter((item) => item.projectId === route.params.id);
    props.value = propResponse.data.filter((item) => item.projectId === route.params.id);
...
async function selectEpisode(episode: Episode) {
  selectedEpisode.value = episode; selectedScene.value = null; selectedShot.value = null; shots.value = []; versions.value = [];
  try {
    const [scriptResponse, sceneResponse] = await Promise.all([
      request<ListResponse<Script>>(`/episodes/${episode.id}/script`), request<ListResponse<Scene>>(`/episodes/${episode.id}/scenes`),
    ]);
    scripts.value = scriptResponse.data; scenes.value = sceneResponse.data;
...
async function selectShot(shot: Shot) {
  ...
  try {
    const [versionResponse, characterResponse] = await Promise.all([request<ListResponse<Version>>(`/shots/${shot.id}/versions`), request<ListResponse<Link>>(`/shots/${shot.id}/characters`)]);
    versions.value = versionResponse.data; assignedCharacters.value = characterResponse.data;
...
async function saveAsset(kind: "character" | "location" | "prop") {
  ...
  try { const body = { projectId: route.params.id, name: assetForm.name, description: nullable(assetForm.description), visualDescription: nullable(assetForm.visualDescription) }; const response = editing[kind] ? await request<SingleResponse<Asset>>(`/${endpoint}/${editing[kind]}`, { method: "PATCH", body }) : await request<SingleResponse<Asset>>(`/${endpoint}`, { method: "POST", body }); collection.value = editing[kind] ? collection.value.map((item) => item.id === editing[kind] ? response.data : item) : [...collection.value, response.data]; editing[kind] = null; assetForm.name = ""; assetForm.description = ""; assetForm.visualDescription = ""; } catch (cause) { formError.value = messageFromError(cause, `Unable to save ${kind}.`); } finally { saving.value = false; }
}
